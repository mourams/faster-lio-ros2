...
bool LaserMapping::LoadParams() {
    ...
    this->declare_parameter<bool>("publish.scan_effect_pub_en", false);
    ...
    this->get_parameter<int>("max_iteration", options::NUM_MAX_ITERATIONS);
    this->get_parameter<float>("esti_plane_threshold", options::ESTI_PLANE_THRESHOLD);
    ...
    this->get_parameter("preprocess.time_scale", preprocess_->TimeScale());
    this->get_parameter("preprocess.lidar_type", lidar_type);
    this->get_parameter("preprocess.scan_line", preprocess_->NumScans());
    this->get_parameter("point_filter_num", preprocess_->PointFilterNum());
    this->get_parameter("feature_extract_enable", preprocess_->FeatureEnabled());
...
void LaserMapping::SubAndPubToROS() {
    ...
    lidar_topic = this->declare_parameter<std::string>("common/lid_topic", "/livox/lidar");
    imu_topic = this->declare_parameter<std::string>("common/imu_topic", "/livox/imu");

    if (preprocess_->GetLidarType() == LidarType::AVIA) {
        sub_pcl_ = this->create_subscription<livox_interfaces::msg::CustomMsg>(
            lidar_topic, 200000, std::bind(&LaserMapping::LivoxPCLCallBack, this, std::placeholders::_1));
    } else {
        sub_pcl_ = this->create_subscription<sensor_msgs::msg::PointCloud2>(
            lidar_topic, 200000, std::bind(&LaserMapping::StandardPCLCallBack, this, std::placeholders::_1));
    }
...
LaserMapping::LaserMapping() : rclcpp::Node("faster_lio") {
...
void LaserMapping::IMUCallBack(const sensor_msgs::msg::Imu::SharedPtr msg_in) {
    ...
    if (abs(timediff_lidar_wrt_imu_) > 0.1 && time_sync_en_) {
        double stamp = timediff_lidar_wrt_imu_ + rclcpp::Time(msg_in->header.stamp).seconds();
        int32_t sec = (int32_t)stamp;
        int32_t nanosec = (int32_t)((stamp - sec) * 1E9);
...
void LaserMapping::PublishPath(const rclcpp::Publisher<nav_msgs::msg::Path>::SharedPtr pub_path) {
    ...
    int32_t sec = (int32_t)lidar_end_time_;
    int32_t nanosec = (int32_t)((lidar_end_time_ - sec) * 1E9);
    msg_body_pose_.header.stamp = rclcpp::Time(sec, nanosec);
...
void LaserMapping::PublishOdometry(const rclcpp::Publisher<nav_msgs::msg::Odometry>::SharedPtr pub_odom_aft_mapped) {
    ...
    int32_t sec = (int32_t)lidar_end_time_;
    int32_t nanosec = (int32_t)((lidar_end_time_ - sec) * 1E9);
    odom_aft_mapped_.header.stamp = rclcpp::Time(sec, nanosec);  // rclcpp::Time().fromSec(lidar_end_time_);
    ...
    t.transform.translation.x = odom_aft_mapped_.pose.pose.position.x;
    t.transform.translation.y = odom_aft_mapped_.pose.pose.position.y;
    t.transform.translation.z = odom_aft_mapped_.pose.pose.position.z;
    t.transform.rotation.w = odom_aft_mapped_.pose.pose.orientation.w;
    t.transform.rotation.x = odom_aft_mapped_.pose.pose.orientation.x;
    t.transform.rotation.y = odom_aft_mapped_.pose.pose.orientation.y;
    t.transform.rotation.z = odom_aft_mapped_.pose.pose.orientation.z;
    t.header.stamp = odom_aft_mapped_.header.stamp;
...
void LaserMapping::PublishFrameWorld() {
    ...
    if (run_in_offline_ == false && scan_pub_en_) {
        ...
        int32_t sec = (int32_t)lidar_end_time_;
        int32_t nanosec = (int32_t)((lidar_end_time_ - sec) * 1E9);
        laserCloudmsg.header.stamp = rclcpp::Time(sec, nanosec);
